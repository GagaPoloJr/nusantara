                ...
                <div id="kt_app_content_container" class="app-container  container-fluid ">
                    <div class="card card-custom card-stretch shadow">
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-row-bordered table-row-dashed gy-4 align-middle" id="menu_datatable">
    ...
    <script>
        $(document).ready(function (){
            setTimeout(function() {
                ...
                $('table#menu_datatable').DataTable({
                    processing: true,
                    serverSide: true,
                    searching:true,
                    /*stateSave:true,*/
                    ajax: {
                        url:`{{ route('menu.datatable.all') }}`,
                ...
                $('button#refresh').on('click', function () {
                    if($('table#menu_datatable').DataTable().settings()[0]['oInit'].serverSide == true){
                        $('table#menu_datatable').DataTable().draw();
                    } else {
                        window.location.reload();
                    }
                });
                $('table#menu_datatable').on('click', 'button#btn_update', function(){
                    let data = $(this).data()
                    let id = data.id
                    $.ajax({
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                ...
                $('table#menu_datatable').on('click', 'button#btn_delete', function(){
                    let data = $(this).data()
                    let id = data.id
