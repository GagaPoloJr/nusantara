                ...
                <div id="kt_app_content_container" class="app-container  container-fluid ">
                    <div class="card card-custom card-stretch shadow">
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-row-bordered table-row-dashed gy-4 align-middle" id="access_datatable">
    ...
    <script>
        $(document).ready(function (){
            setTimeout(function() {
                ...
                $('table#access_datatable').DataTable({
                    processing: true,
                    serverSide: true,
                    searching:true,
                    /*stateSave:true,*/
                    ajax: {
                        url:`{{ route('access.datatable.all') }}`,
                    ...
                    createdRow: function(row, data, dataIndex,cells) {
                        ...
                        $(cells[2]).html('<div class="btn-access  float-end"> @can("delete access") <button id="btn_delete" data-id='+data['access_id']+' type="button" class="btn btn-danger btn-sm waves-effect waves-light float-end me-1 p-2"><span class="btn-label"><i class="fa fa-trash p-1"></i></span>Hapus</button> @endcan @can("update access") <button id="btn_update" data-id='+data['access_id']+' type="button" class="btn btn-info btn-sm waves-effect waves-light float-end me-1 p-2" data-bs-toggle="modal" data-bs-target="#modal_default"><span class="btn-label"><i class="fa fa-pencil p-1"></i></span>Ubah</button> @endcan</div>')
                    },
                ...
                $('button#refresh').on('click', function () {
                    if($('table#access_datatable').DataTable().settings()[0]['oInit'].serverSide == true){
                        $('table#access_datatable').DataTable().draw();
                    } else {
                        window.location.reload();
                    }
                });
                $('table#access_datatable').on('click', 'button#btn_update', function(){
                    let data = $(this).data()
                    let id = data.id
                    $.ajax({
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                ...
                $('table#access_datatable').on('click', 'button#btn_delete', function(){
                    let data = $(this).data()
                    let id = data.id
