                ...
                <div id="kt_app_content_container" class="app-container  container-fluid ">
                    <div class="card card-custom card-stretch shadow">
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-row-bordered table-row-dashed gy-4 align-middle" id="dictionary_datatable">
...
@push('js')
    <script>
        $(document).ready(function (){
            setTimeout(function() {
                $('table#dictionary_datatable').DataTable({
                    processing: true,
                    serverSide: true,
                    searching:true,
                    ajax: `{{ route('dictionary.datatable.all') }}`,
                    searchDelay: 1000,
                    scrollX: !0,
                    ...
                    createdRow: function(row, data, dataIndex,cells) {
                        ...
                        $(cells[2]).html('<div class="btn-dictionary  float-end"> @can("delete dictionary") <button id="btn_delete" data-id='+data['dictionary_id']+' type="button" class="btn btn-danger btn-sm waves-effect waves-light float-end me-1 p-2"><span class="btn-label"><i class="fa fa-trash p-2"></i></span>Hapus</button> @endcan @can("update dictionary") <button id="btn_update" data-id='+data['dictionary_id']+' type="button" class="btn btn-info btn-sm waves-effect waves-light float-end me-1 p-2" data-bs-toggle="modal" data-bs-target="#modal_default"><span class="btn-label"><i class="fa fa-pencil p-2"></i></span>Ubah</button> @endcan</div>')
                    },
                ...
                $('button#refresh').on('click', function () {
                    $('table#dictionary_datatable').DataTable().draw();
                });
                $('table#dictionary_datatable').on('click', 'button#btn_update', function(){
                    let data = $(this).data()
                    let id = data.id
                    $.ajax({
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        method:'GET',
                        url:  `{{ url('dictionary/') }}/${id}/edit`,
                        success: function(res){
                            $('div#modal_default').find('.modal-content').html(res);
                        }
                    });
                });
                $('table#dictionary_datatable').on('click', 'button#btn_delete', function(){
                    let data = $(this).data()
                    let id = data.id
