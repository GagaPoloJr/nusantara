...
<div class="modal-header">
    <h5 class="modal-title" id="scrollableModalTitle">Tambah Akses User {{ $user->name }}</h5>
                ...
                <div class="row">
                    <div class="col-xl-6 col-md-12">
                        ...
                        <div class="card card-custom card-stretch shadow">
                            <div class="card-body">
                                <h4 class="header-title mb-2">Daftar Akses Yang Belum Dimiliki</h4>
                                <table id="nothave_datatable" class="table table-row-bordered table-row-dashed gy-4 align-middle">
                                    ...
                                    <tfood>
                                        <tr>
                                            <td colspan="2"><button type="button" class="btn btn-success btn-sm float-end me-0" id="btn_give"><span class="btn-label"><i class="fa fa-key"></i></span>Tambah</button></td>
                                        </tr>
                                    </tfood>
                                </table>
                            </div> <!-- end card-body -->
                        ...
                        <div class="card card-custom card-stretch shadow">
                            <div class="card-body">
                                ...
                                <table id="have_datatable" class="table table-row-bordered table-row-dashed gy-4 align-middle">
                                    ...
                                    <tfood>
                                        <tr>
                                            <td colspan="2"><button type="button" class="btn btn-danger btn-sm float-end me-0" id="btn_revoke"><span class="btn-label"><i class="fa fa-lock"></i></span>Hapus</button></td>
                                        </tr>
                                    </tfood>
                                </table>
                            </div> <!-- end card-body -->
...
<script>
    $(document).ready(function() {
        $('table#have_datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: `{{ url('user/datatable') }}/{{ $user->user_id }}/access`,
            searchDelay: 1000,
            scrollCollapse: true,
        ...
        $('table#nothave_datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: `{{ url('user/datatable') }}/{{ $user->user_id }}/notaccess`,
            searchDelay: 1000,
            scrollCollapse: true,
            pagingType: "full_numbers",
            lengthMenu: [[5,10,25,50,100,-1],[5,10,25,50,100,'semua']],
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            method:"POST",
                            data:{access_id:id,user_id:`{{ $user->user_id }}`},
