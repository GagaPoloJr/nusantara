<div class="modal-header">
    <h5 class="modal-title" id="scrollableModalTitle"> {{ strtoupper('Pengaturan Perusahaan user '.$user->user_name) }}</h5>
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-xl-6 col-md-12">
                        <div class="card card-custom card-stretch shadow">
                            <div class="card-body">
                                <h4 class="header-title mb-2">Daftar Perusahaan</h4>
...
<script>
    $(document).ready(function () {
        ...
        $('table#notmember_datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: `{{ url('user/datatable') }}/{{ $user->user_id }}/company/notmember`,
            searchDelay: 1000,
            scrollCollapse: true,
            ...
            createdRow: function(row, data, dataIndex,cells) {
                $(cells[0]).html(data['company_name'])
                $(cells[1]).html('@can("create user") <button type="button" class="btn btn-icon btn-light-twitter me-5 float-end" id="btn_join" data-id='+data['company_id']+'><i class="fa fa-person-walking-arrow-right"></i></button> @endcan');
            },
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        url: "{{ route('user.company.store') }}",
                        type: "POST",
                        data: {user_id:user_id,company_id:id},
        ...
        $('table#member_datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: `{{ url('user/datatable') }}/{{ $user->user_id }}/company/member`,
            searchDelay: 1000,
            scrollCollapse: true,
            pagingType: "full_numbers",
            lengthMenu: [[5,10,25,50,100,-1],[5,10,25,50,100,'semua']],
            ...
            createdRow: function(row, data, dataIndex,cells) {
                $(cells[1]).html('@can("create user") <button type="button" class="btn btn-icon btn-light-youtube float-end" id="btn_leave" data-id='+data['company_id']+'><i class="fa fa-person-walking-arrow-right"></i></button> @endcan');
                $(cells[0]).html(data['company_name'])
            },
            drawCallback: function() {
                $(".dataTables_paginate > .pagination").addClass("pagination-rounded")
        ...
        $('table#member_datatable').on('click','button#btn_leave',function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        url: "{{ route('user.company.destroy') }}",
                        type: "POST",
                        data: {user_id:user_id,company_id:id},
                        success:function(response){
                            Swal.fire({
